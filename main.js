...
const achekCommand = require('./commands/achek');
...
const voiceChangeCommand = require('./commands/voicechange');
const bugCommand = require('./commands/bug');
const { questionsCommand, askMeCommand } = require('./commands/questions');
const { truth400Command, categoriesCommand } = require('./commands/truth400');

// Global settings
global.packname = settings.packname;
global.author = settings.author;
global.channelLink = "https://whatsapp.com/channel/0029VbAfyp57dmeXh5trtY2W";
global.supportGroup = "https://chat.whatsapp.com/FO5rtxtFuWBL5EUNc6wEO5";
global.website = "https://achek.com.ng";
global.businessName = "Achek Digital Solutions";
global.groupJid = "120363421505760739@g.us";
global.channelJid = "120363402198872825@newsletter";
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, { 
                    text: '📢 *Join our Channel:*\nhttps://whatsapp.com/channel/0029VbAfyp57dmeXh5trtY2W\n\n_Powered by Achek Digital Solutions_\n🌐 https://achek.com.ng' 
                }, { quoted: message });
                return;
                ...
                await sock.sendMessage(chatId, { 
                    text: `🔗 *Support Group*\n\nhttps://chat.whatsapp.com/FO5rtxtFuWBL5EUNc6wEO5\n\n🏢 *Achek Digital Solutions*\n🌐 https://achek.com.ng` 
                }, { quoted: message });
                return;
            } else if (buttonId === 'achek') {
                await achekCommand(sock, chatId, message);
                return;
            }
        }
        ...
        // List of admin commands (tagall removed - has owner bypass in its handler)
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.roast'):
                ...
                break;
            case userMessage.startsWith('.pickup') || userMessage.startsWith('.pickupline'):
                await pickupLineCommand(sock, chatId, message);
                break;
            case userMessage.startsWith('.funfact') || userMessage.startsWith('.randomfact'):
                await funFactCommand(sock, chatId, message);
                break;
            ...
            case userMessage.startsWith('.sora'):
                ...
                break;
            case userMessage === '.achek' || userMessage === '.achekdigital' || userMessage === '.ads':
                await achekCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            case userMessage.startsWith('.pidgin'):
                {
                    const pidginText = rawText.slice(7).trim();
                    await pidginCommand(sock, chatId, message, pidginText);
                    commandExecuted = true;
                }
                break;
            case userMessage.startsWith('.broadcast'):
                {
                    const broadcastText = rawText.slice(10).trim();
                    await broadcastCommand(sock, chatId, senderId, message, broadcastText);
                    commandExecuted = true;
                }
                break;
            case userMessage === '.rules':
                await rulesCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            ...
            case userMessage === '.uptime' || userMessage === '.runtime':
                ...
                commandExecuted = true;
                break;
            case userMessage.startsWith('.voice') || userMessage.startsWith('.voicechange'):
                {
                    const voiceArgs = userMessage.split(' ').slice(1);
                    await voiceChangeCommand(sock, chatId, message, voiceArgs);
                    commandExecuted = true;
                }
                break;
            case userMessage.startsWith('.bug') || userMessage.startsWith('.spam'):
                {
                    const bugArgs = userMessage.split(' ').slice(1);
                    await bugCommand(sock, chatId, message, bugArgs);
                    commandExecuted = true;
                }
                break;
            case userMessage === '.question' || userMessage === '.q':
                await questionsCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            case userMessage.startsWith('.askme') || userMessage.startsWith('.ask'):
                await askMeCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            case userMessage.startsWith('.truth400') || userMessage.startsWith('.t400'):
                {
                    const truthArgs = userMessage.split(' ').slice(1);
                    await truth400Command(sock, chatId, message, truthArgs);
                    commandExecuted = true;
                }
                break;
            case userMessage === '.categories' || userMessage === '.cats':
                await categoriesCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            default:
                if (isGroup) {
